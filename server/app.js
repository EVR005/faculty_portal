...
app.get("/imageUrl",async (req,res)=>{
  const browser = await puppeteer.launch();
    ...
    const images = await page.$$eval('.img-responsive', anchors => [].map.call(anchors, img => img.src));
...
app.get("/scrapData",async (req,res)=>{
  ...
  // }
    ...
    const result = [];
(async () => {
  const gs_id = ["5puGaPAAAAAJ","rD9h9-gAAAAJ","rTgkrEUAAAAJ","1KdFhYQAAAAJ","JXdJJZ4AAAAJ","qTh3IiYAAAAJ","es-RJ3AAAAAJ","jDKAd6MAAAAJ",
  "X7D4W5kAAAAJ","l9qipyYAAAAJ","_om1c54AAAAJ","QpknulAAAAAJ","VmeFQwUAAAAJ","SoiiKMkAAAAJ","KaZc888AAAAJ","2Nsi39EAAAAJ",
  "S88bYKIAAAAJ","WdfqSlcAAAAJ","7-g7OE8AAAAJ","TSHevDoAAAAJ","Aak9dzcAAAAJ","lsZpAx4AAAAJ","d7SodpkAAAAJ","Zp8-JXEAAAAJ","9IZiUTwAAAAJ","h89DMFcAAAAJ",
  "ojFgyLAAAAAJ","jbvTWngAAAAJ","-Pccm70AAAAJ","60-CQT8AAAAJ","enZRExMAAAAJ","Ywfct0kAAAAJ"];
  for(let i = 0;i < gs_id.length; i++){
    ...
    const browser = await puppeteer.launch();
	const page = await browser.newPage();
    await page.goto('https://scholar.google.com/citations?hl=en&user='+gs_id[i]);
